	...
	<cfif action is "nothing">
		...
		<p>
			...
			<form method="post" action="duplicateLocality.cfm">
				<label for="roundto">Round coordinates to decimal places</label>
				<select name="roundto" id="roundto">
					<cfloop from="1" to="8" index="i">
						<option <cfif roundto is i> selected="selected" </cfif>value="#i#">#i#</option>
					</cfloop>
				</select>
			...
			<cfif DEC_LAT is not "ignore">
				<cfif len(DEC_LAT) gt 0>
					<cfset sql=sql & " round(DEC_LAT,#roundto#)=round(#DEC_LAT#,#roundto#) and ">
				<cfelse>
			...
			<cfif DEC_LONG is not "ignore">
				<cfif len(DEC_LONG) gt 0>
					<cfset sql=sql & " round(DEC_LONG,#roundto#)=round(#DEC_LONG#,#roundto#) and ">
				<cfelse>
