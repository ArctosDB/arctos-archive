...
select preferred_agent_name,
regexp_replace(preferred_agent_name,'([A-Z]\.)([^ ])','\1 \2')
from agent where regexp_like(preferred_agent_name,'[A-Z]\.[^ ]')
and regexp_replace(preferred_agent_name,'([A-Z]\.)([^ ])','\1 \2') in (select preferred_agent_name from agent);

<cfquery name="d" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
	select 
		agent_id,
		preferred_agent_name,
		regexp_replace(preferred_agent_name,'([A-Z]\.)([^ ])','\1 \2') sn
	from 
		agent 
	where 
		regexp_like(preferred_agent_name,'[A-Z]\.[^ ]')
</cfquery>
<cfloop query="d">
<cfquery name="g" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
	select agent_id,preferred_agent_name from agent where preferred_agent_name='#sn#' 
</cfquery>
<cfif g.recordcount gt 0>
	<br>#preferred_agent_name# is a duplicate of #g.preferred_agent_name#
