...
<cfoutput>
	...
	<cfquery name="badSuffix" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
		select suffix from ds_temp_agent where suffix is not null and suffix not in (select suffix from ctsuffix) group by suffix
	</cfquery>
	<cfif badSuffix.recordcount gt 0>
		These suffixes are not code table values. Aborting....
		
		<cfdump var=#badSuffix#>
		
		<cfabort>
	</cfif>
	
	<cfquery name="badPrefix" datasource="user_login" username="#session.dbuser#" password="#decrypt(session.epw,session.sessionKey)#">
		select prefix from ds_temp_agent where prefix is not null and prefix not in (select prefix from ctprefix) group by prefix
	</cfquery>
	<cfif badPrefix.recordcount gt 0>
		
		These prefixes are not code table values. Aborting....
		
		<cfdump var=#badPrefix#>
